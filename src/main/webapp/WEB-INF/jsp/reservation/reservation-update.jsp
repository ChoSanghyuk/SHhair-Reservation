...
<%@ page import="com.shHair.reservation.entity.Reservation"%>

<%@taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>

<%
Calendar cal = Calendar.getInstance();
Reservation theReservation = (Reservation) request.getAttribute("theReservation");
String date;
...
<head>
	<title>SH hair Reservation Update</title>
...
<body >
	...
	<div class = "row">
		...
		<table class = "table table-striped text-center">
			...
			%> 
				<td>
					<input type="radio" class="btn-check" form="date_type_select" name="date" id="<%=s%>" value="<%=s%>" checked>
		...
		<div>
			<select form="date_type_select" name = "type" class="form-select form-select-lg mb-3" aria-label=".form-select-lg example">
				<% if(type.equals("커트")) { %> <option value="커트" selected> 커트 </option> <%} else %> <option value="커트" > 커트 </option>
				<% if(type.equals("파마")) { %> <option value="파마" selected> 파마 </option> <%} else %> <option value="파마" > 파마 </option>
				<% if(type.equals("염색")) { %> <option value="염색" selected> 염색 </option> <%} else %> <option value="염색" > 염색 </option>
			</select>
		</div>
		<div>
